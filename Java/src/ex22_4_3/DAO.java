	...
	public ArrayList<Emp> search(String[] searchword) {
		...
		for (int i = 0; i < searchword.length; i++) {
			if (searchword[i] != null) {
				String single = "";
				if (i == 1 || i == 2 || i == 4)// ename, job, hiredate 는 '필요
					single = "'";
				...
				sql += column_name[i] + "=" + single + searchword[i] + single;
			}
		}

//				}
//					sql = sql + " where" + column_name[i] + "=" + searchword[i]
//							+ " and" + column_name[i] + "=" + searchword[i];
//		
//		}
